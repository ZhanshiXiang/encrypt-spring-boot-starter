package com.study.webopenapi.filters;
...
import com.study.webopenapi.utils.UrlUtil;
...
//        ResponseEnum.BAD_REQUEST.assertIsTrue(RedisUtil.existAny(IpAddressUtil.getIpAddr(request)+nonce));
        ...
        String secret = request.getHeader(AuthConstant.SECRET_KEY);
        // 断言随机数不为空
        ResponseEnum.BAD_REQUEST.assertNotNull(secret);
/*        // 获取数据库密钥信息
...
/*        // 断言过期时间未过期
        ...
        queryStrMap.put(AuthConstant.SECRET_KEY,secret);
        // 断言签名正确
